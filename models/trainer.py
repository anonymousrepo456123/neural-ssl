...
try:
    import deepspeed  # noqa: F401  (only used when ``ds_config`` is supplied)
...
from models.jepa import NDT_JEPA, JEPAOutput
from models.jepa_v2 import NDT_JEPA_v2

# BCI_AudioLLM is the end-to-end audio-LLM model; not used for the
# human-only MAE pretrain + phoneme finetune pipeline reproduced here.
try:
    from models.bci import BCI_AudioLLM  # type: ignore
...
NAME2DATASET = {
    ...
    "multi_ssl": MultiSpikingDataset,  # SSL pretraining across multiple subjects
}

NAME2MODEL = {"NDT": NDT, "NDT_JEPA": NDT_JEPA, "NDT_JEPA_v2": NDT_JEPA_v2}
if BCI_AudioLLM is not None:
    NAME2MODEL["BCI_AudioLLM"] = BCI_AudioLLM

from utils.config import default_trainer_config
from models.model_output import ModelOutput

_JEPA_MODEL_CLASSES = ("NDT_JEPA", "NDT_JEPA_v2")
...
class Trainer():
    def __init__(
        ...
        resume_path = getattr(self.config.training, "resume_from", None)
        if resume_path:
            self.accelerator.load_state(resume_path)
            self.start_epoch = torch.load(os.path.join(resume_path, "epoch.pth"), weights_only=True) + 1
            self.print_v(f"Resuming from {resume_path} (starting at epoch {self.start_epoch}) ...", verbosity=1)
        else:
            self.start_epoch = 1
    ...
    def set_model(self, model, extra_model_kwargs=None):
        ...
        model_kwargs = dict(**self.config.method.model_kwargs)
        model_class_name = getattr(self.config.model, "model_class", "NDT")
        self.is_jepa = model_class_name in _JEPA_MODEL_CLASSES

        if model is None:
            model_class = NAME2MODEL[model_class_name]
            ...
            self.model = model
            self.is_jepa = isinstance(model, (NDT_JEPA, NDT_JEPA_v2))

        self.model.to(self.device_type)
        self.print_v(self.model)

        self.count_parameters(self.model, exclude_layers=["decoder", "embed_dict", "target_encoder"])
    ...
    def build_dataloaders(self):
        ...
        train_sampler = GroupBatchSampler(
            ...
            # Optional per-epoch per-subject cap: balances gradient steps while
            # (unlike a fixed subset) still exposing all trials over many epochs.
            per_epoch_cap=getattr(self.config.training, "train_per_subject_cap", None),
        )
    ...
    def build_optimizer_and_scheduler(self):
        ...
        _trainable = [p for p in self.model.parameters() if p.requires_grad]
    ...
    def prepare_for_distributed_training(self):
        ...
        # Optional model-side hook called after every optimiser step (e.g.
        # JEPA uses this to EMA-update the target encoder). The check has to
        # go through ``unwrap_model`` because accelerate wraps the module.
        unwrapped = self.accelerator.unwrap_model(self.model)
        self._post_step_hook = getattr(unwrapped, "post_optimizer_step", None)
        # And let the model know how many optimiser steps it will see, so any
        # internal schedule (EMA momentum etc.) can be parameterised.
        if hasattr(unwrapped, "set_total_steps"):
            total_steps = (
                self.config.training.num_epochs
                * len(self.train_dataloader)
                // self.config.optimizer.gradient_accumulation_steps
            )
            unwrapped.set_total_steps(total_steps)

    def train(self):
        config = self.config
        global_step = 1
        best_metric = -float("inf")
        best_loss = float("inf")   # used for JEPA (lower loss = better checkpoint)
        train_loss = []
        train_n_examples = []
        train_metrics = {name: [] for name in self.metric_fns.keys()}

        if self.start_epoch > config.training.num_epochs:
            self.print_v(
                f"start_epoch ({self.start_epoch}) > num_epochs ({config.training.num_epochs}): "
                "nothing to train.",
                verbosity=1,
            )
            return
        ...
        for epoch in range(self.start_epoch, config.training.num_epochs+1):
            ...
            for step, (model_inputs, unused_inputs) in enumerate(tqdm(self.train_dataloader)):
                ...
                if (global_step-1) % config.optimizer.gradient_accumulation_steps == 0:
                    ...
                    # Safety-net against NaN/inf from heavy-tailed neural pile
                    # batches; max_norm=50 is large enough to never fire during
                    # normal training but catches catastrophic gradient steps.
                    self.accelerator.clip_grad_norm_(self.model.parameters(), max_norm=50.0)
                    self.optimizer.step()
                    if config.optimizer.scheduler in ["linear", "cosine"]:
                        self.lr_scheduler.step()
                    self.optimizer.zero_grad()
                    # JEPA hook (EMA update of target encoder, etc.)
            ...
            if config.training.eval_every and (epoch % config.training.eval_every == 0):
                ...
                if self.is_jepa:
                    # JEPA: checkpoint on validation loss (lower = better)
                    if config.log_to_wandb and self.accelerator.is_main_process:
                        wandb.log({
                            "epoch": epoch,
                    ...
                    if test_avg_loss < best_loss:
                        ...
                        if config.log_to_wandb and self.accelerator.is_main_process:
                            wandb.log({"best_epoch": epoch, "best_val_loss": best_loss})
                else:
                    # SSL / CTC: metric-based checkpointing (higher = better)
                    ...
                    if config.log_to_wandb and self.accelerator.is_main_process:
                        ...
                        if self.config.method.model_kwargs.method_name in ["ssl"]:
                            wandb.log({
                                "test_population_raster_plot": wandb.Image(gt_pred_fig["plot_gt_pred"]),
                                "test_single_neuron_plot": wandb.Image(gt_pred_fig["plot_r2"]),
                            })
                    ...
                    if val_metric > best_metric:
                        ...
                        if config.log_to_wandb and self.accelerator.is_main_process:
                            wandb.log({"best_epoch": epoch, "best_val_metric": best_metric})
                            if self.config.method.model_kwargs.method_name in ["ssl"]:
                                wandb.log({
                                    "best_population_raster_plot": wandb.Image(gt_pred_fig["plot_gt_pred"]),
                                    "best_single_neuron_plot": wandb.Image(gt_pred_fig["plot_r2"]),
                                })
                ...
                # Optional per-eval hook (e.g. joint per-source best-checkpointing).
                # Called after the combined-val checkpointing; the model may be
                # put in eval() by the hook — self.model.train() below restores it.
                _eval_hook = getattr(self, "_post_eval_hook", None)
    ...
    def evaluate(
        ...
        test_avg_metrics = {k: sum(v)/len(v) for k, v in test_metrics.items() if v}
