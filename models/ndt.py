...
from utils.datasets import DATASET_CHANNELS, DATASET_TO_IDX
...
class NDTOutput(ModelOutput):
    ...
    latents: Optional[torch.FloatTensor] = None    # (B, T_p, D) encoder output
    patch_mask: Optional[torch.BoolTensor] = None  # (B, T_p) patch-level mask
...
class ReadIn(nn.Module):
    ...
    def __init__(
        ...
        # Build dataset-specific embedding layers, keyed by the *global*
        # DATASET_TO_IDX value of each dataset (rather than a positional
        # enumeration). This makes the read-in module robust to multiple
        # subjects/datasets being registered in a non-contiguous range.
        self.embed_dict = nn.ModuleDict({
            str(DATASET_TO_IDX[name]): self.patch_embedder(channel_count, hidden_size, patch_size)
            for name, channel_count in channel_dict.items()
        })
...
class ReadOut(nn.Module):
    ...
    def __init__(
        ...
        # Keyed by the *global* DATASET_TO_IDX value of each dataset (see ReadIn).
        self.readout_dict = nn.ModuleDict({
            str(DATASET_TO_IDX[name]): (
...
class NeuralEmbeddingLayer(nn.Module):
    ...
    def masker(
        ...
        if torch.bernoulli(torch.tensor(self.expand_prob).float()):
            ...
            span = 1
        span = min(span, num_patches)  # guard against trials shorter than span
...
class NDT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        channel_dict = features if isinstance(features, dict) else DATASET_CHANNELS[features]
        ...
        if encoder_pt_path is not None:
            ...
            # Always finetune with bidirectional attention regardless of pretraining context.
            # AR models pretrain causally (forward=0) but finetuning should be bidirectional.
            config["encoder"]["context"]["forward"] = -1
        ...
        if encoder_pt_path is not None:
            enc_sd = torch.load(os.path.join(encoder_pt_path, "encoder.bin"))
            # Optionally skip the per-dataset read-in weights so they start from a
            # fresh init (diagnostic for negative transfer: keeps the pretrained
            # encoder body but re-learns the read-in for the finetuning subject).
            if kwargs.get("reinit_readin", False):
                # keys are e.g. "embedder.read_in.embed_dict.<idx>.0.weight"
                _dropped = [k for k in list(enc_sd) if "read_in." in k]
                for k in _dropped:
                    enc_sd.pop(k)
                print(f"reinit_readin=True -> dropped {len(_dropped)} read_in.* keys "
                      f"(read-in freshly initialized)")
            self.encoder.load_state_dict(enc_sd, strict=False)

        if kwargs.get("freeze_encoder", False):
            self.encoder.requires_grad_(False)
            print("freeze_encoder=True -> encoder frozen, only decoder will be trained")
    ...
    def forward(
        ...
        latents = None
        patch_mask_eval = None

        if self.method in ["ssl"]:
            latents = x                                                  # (B, T_p, D)
            patch_mask_eval = targets_mask.bool() & spikes_mask.bool()  # (B, T_p)
...
def eval_rankme(model, model_inputs, unused_inputs, outputs, config, **kwargs):
    """RankMe (Garrido et al. 2023) on the encoder latents at masked
    positions: exp(entropy of the normalised singular-value spectrum).

    Computed on the centered token matrix of one eval batch (~thousands of
    tokens × D dims → a fast SVD). Higher = features fill more dimensions.

    Works for both MAE (uses ``latents``/``patch_mask`` fields) and JEPA
    (falls back to ``targets``/``mask`` which hold the EMA-target latents).
    """
    target_arr = outputs.get("latents") if outputs.get("latents") is not None else outputs.get("targets")
    mask_arr = outputs.get("patch_mask") if outputs.get("patch_mask") is not None else outputs.get("mask")
    if target_arr is None or mask_arr is None:
        return torch.tensor(float("nan"), device=model_inputs["spikes"].device)
    target = target_arr.float().detach().cpu().numpy()   # (B, T_p, D)
    mask = mask_arr.detach().cpu().numpy().astype(bool)  # (B, T_p)
    X = target[mask]                                     # (N, D)
    if X.shape[0] < 16:
        return torch.tensor(float("nan"), device=model_inputs["spikes"].device)
    Xc = X - X.mean(axis=0, keepdims=True)
    # subsample for SVD memory if a batch is unusually large
    if Xc.shape[0] > 20000:
        idx = np.random.RandomState(0).choice(Xc.shape[0], 20000, replace=False)
        Xc = Xc[idx]
    ...
    return torch.tensor(rankme, device=model_inputs["spikes"].device)
