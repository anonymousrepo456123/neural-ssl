"""Preprocessing for the Even-Chen dataset (M1+PMd, continuous spike times, NWB).
...
Subjects:
    sub-JenkinsC  — 5 sessions, 192 fixed units
    sub-Reggie    — 7 sessions, 192 fixed units

The 192 units correspond to 2 Utah arrays of 96 channels each (M1 + PMd),
electrode IDs 0–191 identical across all sessions per subject.

Usage:
    python prepare_evenchen.py --out_dir ./data/evenchen/processed
...
def bin_unit(spike_times: np.ndarray, t_max: float) -> np.ndarray:
    """Spike times (seconds) → array of counts at 20ms, shape (T,)."""
    n_bins  = int(np.ceil(t_max / BIN_S))
    bin_idx = np.floor(spike_times / BIN_S).astype(np.int64)
    bin_idx = np.clip(bin_idx, 0, n_bins - 1)
    return np.bincount(bin_idx, minlength=n_bins).astype(np.float32)


def bin_session(fp: Path) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Read an NWB file and return (counts, unit_ids, t_start_trial, t_stop_trial).

    counts: (N_units, T) float32, t_max determined from the last spike time.
    unit_ids: (N_units,) int, sorted by ID.
    t_start/stop: (N_trials,) seconds.
    """
    ...
    # Reconstruct spike times per unit
    ...
    # Sort by unit ID (usually already sorted, but just in case)
    order  = np.argsort(uid)
    return counts[order], uid[order], t_start, t_stop


# ── global statistics (online formula) ───────────────────────────────────────

def compute_subject_stats(files: List[Path]) -> Tuple[np.ndarray, np.ndarray]:
    """Compute per-unit mean/std pooling all sessions for this subject.
    ...
    for fp in files:
        ...
        counts, _, _, _ = bin_session(fp)   # (192, T)
        sum_x  += counts.sum(axis=1).astype(np.float64)
        sum_x2 += (counts ** 2).sum(axis=1).astype(np.float64)
...
def extract_trials(
    ...
    counts, _, t_start, t_stop = bin_session(fp)   # (192, T)
...
def zscore_and_save(
    ...
    """Preprocess all Even-Chen subjects and save NPZ + index JSONL.
    ...
    for subj_dir in sorted(data_dir.iterdir()):
        ...
        if not files:
            continue
        subj = subj_dir.name
        print(f'\n[evenchen] {subj}: {len(files)} sessions')
        ...
        for split, split_fs in splits_files.items():
            for day_idx, fp in enumerate(split_fs):
                date = fp.name[len(subj)+5:len(subj)+5+8]   # YYYYMMDD
            ...
            n_trials = sum(1 for r in all_records[split] if r['subject'] == subj)
            print(f'  {split}: {n_trials} trials')
    ...
    for split, recs in all_records.items():
        ...
        print(f'\n[evenchen] {split} total: {len(recs)} trials')

    print(f'\n[evenchen] done. Output in {out_dir}')
...
DATASET_IDX_MAP: Dict[str, int] = {
    "sub-JenkinsC": 32,   # → neural_pile_sub-JenkinsC_even-chen
    "sub-Reggie":   49,   # → neural_pile_sub-Reggie_even-chen
}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir',    default='./data/evenchen/processed')
